...
def find_sweeps(candles: list[Candle], lookback: int = 5) -> list[LiquiditySweep]:
    ...
    for i in range(lookback, len(candles)):
        ...
        if c.high > max(highs) and c.close < max(highs): out.append(LiquiditySweep("sell", max(highs), i))
        if c.low < min(lows) and c.close > min(lows): out.append(LiquiditySweep("buy", min(lows), i))
    return out
